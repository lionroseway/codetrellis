...
## Project Overview
A web/desktop app (codetrellis.dev) that monitors AI coding agents (starting with Claude Code) and visualizes their impact on codebase architecture in real-time. Provides a dependency graph of codebases (packages → files → classes/functions), overlays agent plans and changes, and lets developers understand what an AI agent is doing before/during/after it acts.
...
- **Agent comms**: Local MCP server (SSE on :19432) + Claude Code JSONL watcher
