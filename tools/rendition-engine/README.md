...
| `stub-check.c` | Links against `no-network.js` with memory past 2 GB, as LibreOffice's build does, and calls every stubbed function. `build.sh` runs it first. A stub that doesn't fit its call then fails in minutes, not at the final link hours in, and any call that succeeds fails the build. |
