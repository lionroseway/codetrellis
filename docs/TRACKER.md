...
| Learn Trellis (in-app onboarding) | 70 | Medium | Apr 28: v1 shipped, v1.1 reframed around real workflows, v1.2 added the agent toolbox step. Nine-step full-screen takeover (`src/frontend/components/LearnTrellis.tsx`) — **Welcome** → **Watching live** (just see diffs, no plan) → **Building a plan** (templates + AI-assisted authoring) → **Adjusting** (edit / AI-revise mid-flight) → **Sharing** (export to git, auto-sync round-trip) → **Pointing agents** (MCP snippet + scoping prompts to a phase, with skill-resource priming tip) → **The agent toolbox** (lists the 30+ MCP tools by category + skill resources at `codetrellis://skill[/quickstart\|/power-user]` so users know what to ask for) → **Refreshing context** (agent re-reads plan via MCP tools) → **The four modes** (Live / Baseline / Planned / Diff). Auto-opens once on first launch (when `localStorage.codetrellis:learn-trellis:seen` isn't set AND no projects yet); re-openable any time from the TopBar GraduationCap button. Backdrop click + Esc + arrow-key nav + skippable + clickable dots. State lives on `ui-store.learnTrellisOpen`. **v1.x is information-only** — no UI element spotlighting / interactive demos. v2 layers spotlight tour mechanics on top. |
