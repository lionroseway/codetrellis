...
| Electron desktop build | 95 | High | `npm run make` produces a working DMG (~103 MB) with the standard `CodeTrellis.app` ↔ `Applications` drag layout. Backend embedded; loopback-only (127.0.0.1); CORS for `file://`; sql.js + web-tree-sitter shipped via `extraResource`; tree-sitter WASM grammars same. Build metadata generated at packaging time (`scripts/generate-build-info.js`) and exposed via `/api/build-info` + Settings → About (version, build #, commit, branch, dirty flag, copy-to-clipboard). File logging at `<dataDir>/logs/<YYYY-MM-DD>.log` with daily rollover; Settings → Logs shows live tail + Reveal in Finder. Windows installer config in place via maker-squirrel — needs a Windows host or CI matrix. Code-signing scaffolding conditional on env vars. Outstanding: auto-updater (electron-updater + GitHub Releases), GitHub Actions release workflow, real signing certs. |
