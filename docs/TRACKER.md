...
| Learn Trellis (in-app onboarding) | 65 | Medium | Apr 28: v1 shipped, then v1.1 same day reframed around real workflows. Eight-step full-screen takeover (`src/frontend/components/LearnTrellis.tsx`) — **Welcome** → **Watching live** (just see diffs, no plan) → **Building a plan** (templates + AI-assisted authoring) → **Adjusting** (edit / AI-revise mid-flight) → **Sharing** (export to git, auto-sync round-trip) → **Pointing agents** (MCP snippet + scoping prompts to a phase) → **Refreshing context** (agent re-reads plan via MCP tools) → **The four modes** (Live / Baseline / Planned / Diff). Each step maps to a concrete user workflow rather than abstract feature description. Auto-opens once on first launch (when `localStorage.codetrellis:learn-trellis:seen` isn't set AND no projects yet); re-openable any time from the TopBar GraduationCap button. Backdrop click + Esc + arrow-key nav + skippable + clickable dots. State lives on `ui-store.learnTrellisOpen`. **v1.x is information-only** — no UI element spotlighting, no interactive demos. v2 layers spotlight tour mechanics on top. |
