...
| Learn Trellis (in-app onboarding) | 60 | Medium | Apr 28: v1 shipped. Six-step full-screen takeover (`src/frontend/components/LearnTrellis.tsx`) — Welcome → Open a project → Author a plan → Connect agent (with copyable MCP snippet) → Watch the agent work → You're set up. Auto-opens once on first launch (when `localStorage.codetrellis:learn-trellis:seen` isn't set AND no projects yet); re-openable any time from the new TopBar GraduationCap button. Backdrop click + Esc + arrow-key nav + skippable. State lives on `ui-store.learnTrellisOpen` so any surface can trigger it. **v1 is information-only** — no UI element spotlighting, no interactive demos, just clean focused screens. v2 (later) layers spotlight tour mechanics on top. |
