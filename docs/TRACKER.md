...
| **Multi-agent broken at the wire level.** The backend's `mcpServer` is a singleton and `Server.connect(transport)` in `@modelcontextprotocol/sdk` is single-transport. The second SSE client to connect re-binds the singleton, severing the first one's stream ("other side closed"). The Connected Agents widget + multi-agent timeline + `claim_task` contention all rely on multi-agent working — and it doesn't. The TopBar widget has been "rendering" sessions because they're tracked separately in `sessionService`, but only ONE agent can actually exchange MCP messages at a time. | `tests/e2e/multi-agent.test.ts` (both tests `test.fixme()` until fixed) | M — extract the tool-registration code in `mcp/server.ts` into `registerTools(server)`, create a fresh `McpServer` per SSE connection inside the `/sse` handler, register tools onto it, then connect the per-connection transport. ~1 day. | ❌ open |
| **Plan-file-watcher (Phase 13 §B auto-sync) flakes ~30%.** chokidar v4 + `awaitWriteFinish` + `ignoreInitial: true` against a directory created shortly after watch start has races on macOS — first write after dir creation sometimes lands inside chokidar's "initial" window. The auto-sync UX claim ("agent edits YAML directly and UI updates without re-scan") is shipped at "100%" but only works most of the time. | `tests/e2e/plan-export.test.ts` (chokidar variant `test.fixme()`; explicit-import path is deterministic and green) | S — drop `awaitWriteFinish`, pre-mkdir `<project>/.codetrellis/plans/` on backend start so chokidar binds to an existing path, or add an explicit re-scan-on-add fallback. ~half day. | ❌ open |
